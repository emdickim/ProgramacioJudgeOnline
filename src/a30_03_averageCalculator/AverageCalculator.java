...
public class AverageCalculator {
    ...
    public double averageCalculator(double[] values) {
        if (values == null) throw new NullPointerException("Array can't be null.");
        if (values.length == 0) throw new ArithmeticException("Array can't be empty");
        double sum = 0;
        for (int i = 0; i < values.length; i++) {
            if (values[i] < 0) throw new IllegalArgumentException("Array can't be negative");
            sum += values[i];
        }
